...
from src.data_folder.manage_datasets import create_datasets, load_datasets, add_to_datasets


def data_creation_for_loop(base_train=None, base_val=None):
    if base_train == None and base_val == None:
        create_valTrain_folder('src/data_folder/test_dataloader',n=10, randomize=True)
        create_datasets('base_dataset')
        base_train, base_val = load_datasets('base_dataset')
    

    create_valTrain_folder('src/data_folder/test_dataloader',n=40, randomize=True)
...
def endless_loop():
    ...
    if torch.load('src/neural_network_stuff/models/endless_loop_model') == None:
        model = train_model_on_loop_dataset(base_train, base_val, modell=None, criterion=criterion)
    else:
        model.load_state_dict(torch.load('src/neural_network_stuff/models/endless_loop_model'))
