...
from visualize.draw_graph import test_drawer
...
def get_label(id):
    data = open('data_folder/get_system_image/img/label.txt', 'r').read().split('\n')
    for element in data:
...
def create_label_for_cut_images(id,old_label, ABSTAND = 200):
    label_file = f'data_folder/cut_images/label.txt'
...
def resize(id='5af694520f5f'):
    ...
    path = f'data_folder/get_system_image/img/{id}.jpg'
    out_path = f'data_folder/cut_images/{id}.jpg'
    try:
        label = get_label(id)
...
def resize_all_images():
    paths = os.listdir('data_folder/get_system_image/img')
    for path in paths:
        if path != 'label.txt':
            id = path.split('.')[0]
            resize(id)
